...
def get_checkpoints(early_arrival, late_arrival, service_duration, start_node):
    if start_node.is_end_depot():
        ...
        checkpoints = get_no_idling_checkpoints(early_arrival, late_arrival, service_duration, opening_hours)
        if checkpoints:
            return checkpoints, False
        checkpoints = get_idling_checkpoints(early_arrival, late_arrival, service_duration, opening_hours)
        return checkpoints, True


def get_no_idling_checkpoints(early_arrival, late_arrival, service_duration, opening_hours):
    checkpoints = []
    for arrival_time in (range(early_arrival, late_arrival + 1)):
        checkpoint = get_checkpoint(arrival_time, arrival_time, service_duration, opening_hours)
        if checkpoint:
            checkpoints.append(checkpoint)
    return checkpoints


def get_idling_checkpoints(early_arrival, late_arrival, service_duration, opening_hours):
    checkpoints = []
    for arrival_time in range(early_arrival, late_arrival + 1):
        service_start_time = arrival_time
        checkpoint = get_checkpoint(arrival_time, service_start_time, service_duration, opening_hours)
...
def get_checkpoint(arrival_time, service_start_time, service_duration, opening_hours):
...
def print_arc_info(start_node, end_node, distance, start_time, early, late, service, checkpoints, verbose):
    if verbose:
        print(f'Legal arc: {start_node} -> {end_node} | Distance: {distance} | Departure: {start_time} | '
