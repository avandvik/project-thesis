            ...
            for start_node in data.ALL_NODES[:-1]:
                ...
                for end_node in data.ALL_NODES[1:]:
                    ...
                    for start_time in start_times:
                        end_times, costs, arr_times, idling = hlp.get_arc_data(start_node, end_node, start_time, vessel)
                        if not end_times:
                            continue
                        self.save_arcs(start_node, end_node, start_time, arr_times, end_times, costs, vessel, idling)

    def save_arcs(self, start_node, end_node, start_time, arr_times, end_times, costs, vessel, idling):
        if end_node.is_end_depot() or idling:
            self.add_best_arc(start_node, end_node, costs, start_time, arr_times, end_times, vessel)
        else:
...
if __name__ == '__main__':
    ag = ArcGeneratorS(verbose=True)
    ag.generate_arcs()
    ag.print_arcs()


"""
shortest_distance = data.DEPOT.get_distance_to_installation(data.ALL_NODES[1].get_installation())
for order_node in data.ALL_NODES[2:-1]:
    distance = data.DEPOT.get_distance_to_installation(order_node.get_installation())
    if distance < shortest_distance:
        shortest_distance = distance
min_sailing_duration = hlp.get_min_sailing_duration(shortest_distance)
latest_start_time = data.PERIOD_DISC - min_sailing_duration * 2
return [start_time for start_time in range(earliest_start_time, latest_start_time)]
"""
