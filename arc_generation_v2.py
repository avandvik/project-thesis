...
class ArcGenerator:
    ...
    def generate_arcs(self, preparation_end_time):
        # TODO: Add depot as first and final element in orders and update indexing of orders
...
def get_arrival_time_span(distance, departure_time):
    ...
    # TODO: Adapt weather to correct timing (hourly)
    speed_impacts = [data.SPEED_IMPACTS[w] for w in data.WEATHER_FORECAST[departure_time:latest_arrival_time + 1]]
    ...
    while sailed_distance < distance:
        sailed_distance += adjusted_max_speeds[departure_time + min_sailing_duration] * data.DISCRETIZED_TIME_UNIT
